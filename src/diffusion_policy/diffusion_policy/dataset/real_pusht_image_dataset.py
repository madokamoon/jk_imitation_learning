...
class RealPushTImageDataset(BaseImageDataset):
    def __init__(self,
        ...
        # print("episode 0")
        # for i in range(277):
        #     cv2.imshow("0", replay_buffer.data.get("camera_0")[i])
        #     cv2.imshow("1", replay_buffer.data.get("camera_1")[i])
        #     cv2.waitKey(50)
        # print("episode 1")
        # for i in range(277, 502):
        #     cv2.imshow("0", replay_buffer.data.get("camera_0")[i])
        #     cv2.imshow("1", replay_buffer.data.get("camera_1")[i])
        #     cv2.waitKey(50)
        # print("episode 2")
        # for i in range(134, 281):
        #     cv2.imshow("0", replay_buffer.data.get("camera_0")[i])
        #     cv2.imshow("1", replay_buffer.data.get("camera_1")[i])
        #     cv2.waitKey(50)

        a = np.concatenate((replay_buffer.data.get("action")[:, 0].reshape(-1, 1), replay_buffer.data.get("robot_eef_pose")[:, 0].reshape(-1, 1)), axis=1)
        d = replay_buffer.data.get("action")[:-1, 0] - replay_buffer.data.get("robot_eef_pose")[1:, 0]
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    idxs_choice = [0, 1, 2, 3, 4, 5, 6]
    # transform lowdim dimensions
    # if action_shape == (2,):
        # 2D action space, only controls X and Y
    zarr_arr = replay_buffer['action']
    zarr_resize_index_last_dim(zarr_arr, idxs=idxs_choice)
    idxs_choice = [0, 1, 2, 3, 4, 5]
